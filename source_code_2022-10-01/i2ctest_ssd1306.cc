 ...
 * 動作しない場合は87行目あたりの#define I2C_DEVICE "/dev/i2c-2"の部分の"/dev/i2c-2"を"/dev/i2c-1"に変更してみてくさい。
 *
 ...
 *
 * コンパイル方法
 * $ g++ -O2 -I/usr/include/freetype2 -o i2ctest_ssd1306 i2ctest_ssd1306.cc -lfreetype
 *
 *
 * 実行方法
 *
 ...
 * )
 *
 * 214行目あたりのssd1306_set_font()の部分で日本語フォントを指定すれば日本語の表示も可能です。
 * 日本語はssd1306_word_print()ではなく、ssd1306_print()で表示してください。
 * 
 * mango pi mq priとSSD1306 OLEDの接続はこんな感じです
 *
 * ピンはpin 32(PB0 ピンヘッダの15番ピン)が/dev/i2c-1のSCL
 * ピンはpin 33(PB1 ピンヘッダの16番ピン)が/dev/i2c-1のSDA
 *
 *
...
void loop() {
    ...
    // 文字の表示
    ssd1306_word_print(&ssd1306_ft_face01, "MangoPi Mq Pro\nHello World!!\nSSD1306\n");
    // 描画した内容をSSD1306 OLEDに反映させる
    ssd1306_display();
    delay(5000);
