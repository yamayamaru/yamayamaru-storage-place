...
static int16 replicateSignBit16(int8 n)
{
   switch (n)
   {
      case 0:  return (uint16_t)0x0000;
      case 1:  return (uint16_t)0x8000;
      case 2:  return (uint16_t)0xC000;
      case 3:  return (uint16_t)0xE000;
      case 4:  return (uint16_t)0xF000;
      case 5:  return (uint16_t)0xF800;
      case 6:  return (uint16_t)0xFC00;
      case 7:  return (uint16_t)0xFE00;
      case 8:  return (uint16_t)0xFF00;
      case 9:  return (uint16_t)0xFF80;
      case 10: return (uint16_t)0xFFC0;
      case 11: return (uint16_t)0xFFE0;
      case 12: return (uint16_t)0xFFF0; 
      case 13: return (uint16_t)0xFFF8;
      case 14: return (uint16_t)0xFFFC;
      case 15: return (uint16_t)0xFFFE;
...
/* //#define PJPG_MAX_IN_BUF_SIZE 1024 */
...
static uint16 getBits(uint8 numBits, uint8 FFCheck)
{
   ...
   if (numBits > (uint8)8)
   {
      ...
      gBitBuf = gBitBuf << gBitsLeft;
      
      gBitBuf = gBitBuf | getOctet(FFCheck);
      
      gBitBuf = gBitBuf << (8 - gBitsLeft);
   ...
   if (gBitsLeft < numBits)
   {
      ...
      gBitBuf = gBitBuf << (numBits - gBitsLeft);
...
static PJPG_INLINE uint8 getBit(void)
{
   ...
   if (!gBitsLeft)
   {
      gBitBuf = gBitBuf | getOctet(1);

      gBitsLeft = gBitsLeft + 8;
   }
...
static uint16 getExtendTest(uint8 i)
{
   switch (i)
   {
      ...
      case 1: return (uint16_t)0x0001;
      case 2: return (uint16_t)0x0002;
      case 3: return (uint16_t)0x0004;
      case 4: return (uint16_t)0x0008;
      case 5: return (uint16_t)0x0010; 
      case 6: return (uint16_t)0x0020;
      case 7: return (uint16_t)0x0040;
      case 8:  return (uint16_t)0x0080;
      case 9:  return (uint16_t)0x0100;
      case 10: return (uint16_t)0x0200;
      case 11: return (uint16_t)0x0400;
      case 12: return (uint16_t)0x0800;
      case 13: return (uint16_t)0x1000;
      case 14: return (uint16_t)0x2000; 
      case 15: return (uint16_t)0x4000;
...
*/
   for ( ; ; )
   {
      ...
      if ((code <= maxCode) && (maxCode != (uint16)0xFFFF))
         break;
...
static uint8 readDHTMarker(void)
{
   ...
   while (left)
   {
      ...
      gValidHuffTables = gValidHuffTables | (1 << tableIndex);
      ...
      if (count > (uint16)getMaxHuffCodes(tableIndex))
...
static uint8 readDQTMarker(void)
{
   ...
   while (left)
   {
      ...
      gValidQuantTables = gValidQuantTables | (n ? 2 : 1);         
...
static uint8 readSOFMarker(void)
{
   ...
   if ((!gImageYSize) || (gImageYSize > (uint16)PJPG_MAX_HEIGHT))
      return PJPG_BAD_HEIGHT;

   gImageXSize = getBits1(16);

   if ((!gImageXSize) || (gImageXSize > (uint16)PJPG_MAX_WIDTH))
      return PJPG_BAD_WIDTH;

   gCompsInFrame = (uint8)getBits1(8);

   if (gCompsInFrame > (uint8)3)
...
static uint8 locateSOIMarker(void)
{
   ...
   for ( ; ; )
   {
      ...
      if (lastchar == (uint8)0xFF) 
      {
         if (thischar == (uint8)M_SOI)
            break;
         else if (thischar == (uint8)M_EOI)	/* //getBits1 will keep returning M_EOI if we read past the end */
            return PJPG_NOT_JPEG;
      }
   }
...
static uint8 checkHuffTables(void)
{
   ...
   for (i = 0; i < gCompsInScan; i++)
   {
      compDCTab = gCompDCTab[gCompList[i]];
      compACTab = gCompACTab[gCompList[i]] + 2;
...
static uint8 checkQuantTables(void)
{
   ...
   for (i = 0; i < gCompsInScan; i++)
   {
      compQuantMask = gCompQuant[gCompList[i]] ? 2 : 1;
...
static uint8 initScan(void)
{
   uint8 foundEOI;
   uint8 status;
   status = locateSOSMarker(&foundEOI);
   if (status)
      return status;
   if (foundEOI)
      return PJPG_UNEXPECTED_MARKER;
...
{
   if (gCompsInFrame == (uint8)1)
   {
      if ((gCompHSamp[0] != (uint8)1) || (gCompVSamp[0] != (uint8)1))
   ...
   else if (gCompsInFrame == (uint8)3)
   {
      if ( ((gCompHSamp[1] != (uint8)1) || (gCompVSamp[1] != (uint8)1)) ||
         ((gCompHSamp[2] != (uint8)1) || (gCompVSamp[2] != (uint8)1)) )
         return PJPG_UNSUPPORTED_SAMP_FACTORS;
      ...
      else if ((gCompHSamp[0] == (uint8)1) && (gCompVSamp[0] == (uint8)2))
      {
      ...
      else if ((gCompHSamp[0] == (uint8)2) && (gCompVSamp[0] == (uint8)1))
      {
      ...
      else if ((gCompHSamp[0] == (uint8)2) && (gCompVSamp[0] == (uint8)2))
      {
...
static void idctRows(void)
{
   ...
   for (i = 0; i < 8; i++)
   {
      if ((pSrc[1] | pSrc[2] | pSrc[3] | pSrc[4] | pSrc[5] | pSrc[6] | pSrc[7]) == (int16)0)
      {
/*
...
static void idctCols(void)
{
   ...
   for (i = 0; i < 8; i++)
   {
      if ((pSrc[1*8] | pSrc[2*8] | pSrc[3*8] | pSrc[4*8] | pSrc[5*8] | pSrc[6*8] | pSrc[7*8]) == (int16)0)
      {
/*
         // Short circuit the 1D IDCT if only the DC component is non-zero
*/
         c = clamp(PJPG_DESCALE(*pSrc) + 128);
         *(pSrc+0*8) = c;
         *(pSrc+1*8) = c;
         *(pSrc+2*8) = c;
         *(pSrc+3*8) = c;
...
*/
   ...
   pSrc = gCoeffBuf + srcOfs;
   pDstG = gMCUBufG + dstOfs;
   pDstB = gMCUBufB + dstOfs;
   for (y = 0; y < 4; y++)
   {
      for (x = 0; x < 4; x++)
      {
...
static void copyY(uint8 dstOfs)
{
   ...
   pRDst = gMCUBufR + dstOfs;
   pGDst = gMCUBufG + dstOfs;
   pBDst = gMCUBufB + dstOfs;
   pSrc = gCoeffBuf;
...
static void transformBlockReduce(uint8 mcuBlock)
{
   ...
   c = clamp(PJPG_DESCALE(gCoeffBuf[0]) + 128);
...
static uint8 decodeNextMCU(void)
{
   ...
   for (mcuBlock = 0; mcuBlock < gMaxBlocksPerMCU; mcuBlock++)
   {
      ...
      s = huffDecode(compDCTab ? &gHuffTab1 : &gHuffTab0, compDCTab ? gHuffVal1 : gHuffVal0);
...
*/
         for (k = 1; k < 64; k++)
         {
            ...
            else
            {
               if (r == (uint16)15)
               {
                  if ((k + (uint8)16) > (uint8)64)
                     return PJPG_DECODE_ERROR;

                  k = k + (16 - 1); /*// - 1 because the loop counter is k */
               }
               else
                  break;
            }
