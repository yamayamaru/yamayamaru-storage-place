...
#undef random
#endif
#define random(num)     (rand() % (num))	/* fast version */
...
int main()
{
  ...
  if( MaxColors >= 16 &&
      ( GraphDriver==EGA || GraphDriver==EGALO || GraphDriver==VGA ||
        GraphDriver==DOSVGA || GraphDriver==PC98 || GraphDriver==PC98HI ) )
    PaletteDemo();
  PutPixelDemo();
  PutImageDemo();
  Bar3DDemo();
...
void PutImageDemo(void)
{
  ...
  /* Draw Saucer */
  setfillstyle( SOLID_FILL, getmaxcolor()-3 );
