...
#define DBUFFER_WIDTH       (640)
#define DBUFFER_HEIGHT      (480)
...
int main(int argc, char *argv[]) {
    ...
    if ((fp1 = fopen("bmppale1.dat", "rb")) == NULL) {
        set_color(255);
    ...
    // 画像の読み込み(データの読み込み)
    if ((fp1 = fopen("bmpdata1.dat", "rb")) == NULL) {
        set_color(255);
        draw_string(0, 20, "ファイルが開けません");
        exit(1);
    }
...
void screen_update(void) {
    ...
    for (i = 0; i < DBUFFER_HEIGHT; i++) {
        copy_mem_to_vram(mem_addr, vram_addr, DBUFFER_WIDTH);
...
void copy_vram(void) {
    ...
    mem_addr = double_buffer;
    for (i = 0; i < DBUFFER_HEIGHT; i++) {
        copy_vram_to_mem(vram_addr, mem_addr, DBUFFER_WIDTH);
        mem_addr += DBUFFER_WIDTH;
        vram_addr += VRAM_SCREEN_WIDTH;
    }
}
