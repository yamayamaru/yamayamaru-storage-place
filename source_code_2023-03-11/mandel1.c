...
#define VRAM_BLUE  0xa8000
#define VRAM_RED   0xb0000
#define VRAM_GREEN 0xb8000
#define VRAM_INT   0xe0000
...
void pset(int x, int y, int color) {
    ...
    p = (char *)VRAM_BLUE + addr01;
    if (color & 0x01) {
        *p = *p | c;
    } else {
        *p = *p & ~c;
    }
    p = (char *)VRAM_GREEN + addr01;
    if (color & 0x02) {
        *p = *p | c;
    } else {
        *p = *p & ~c;
    }
    p = (char *)VRAM_RED + addr01;
    if (color & 0x04) {
        *p = *p | c;
    } else {
        *p = *p & ~c;
    }
    p = (char *)VRAM_INT + addr01;
    if (color & 0x08) {
        *p = *p | c;
    } else {
        *p = *p & ~c;
