LicheePi4Aでlibgpiodv2.0を使ってLチカをする


libgpiodを使うのですがaptで入るlibgpiodでは動作しないので自分でビルドしてインストールします

libgpiodはここにあるlibgpiod-2.0.tar.gzをダウンロードします
...
ビルドします(ここでは/usr/localにたくさんファイルがあるのでインストール先にmylib_localを設定してます
あとでLD_LIBRARY_PATHでlibのサーチパスを設定する必要があります)
...
gcc -O2 -I/home/sipeed/mylib_local/include -L/home/sipeed/mylib_local/lib -o test_libgpiod_gpio01 test_libgpiod_gpio01.c -lgpiod




libgpiod-2.0が/home/sipeed/mylib_localに入ったのでlibgpiod-2.0を使ったプログラムを実行するには毎回下記を実行します
(.profileに追記するといいかもしれないです)
...
gpiochip4のアクセス権を設定します
$ sudo chmod o+rw /dev/gpiochip4



実行は下記のようにtest_libgpiod_gpio01-exec.shという名前でスクリプトを作って実行すると便利です
...
$ chmod u+x test_libgpiod_gpio01-exec.sh



スクリプトの実行

$./test_libgpiod_gpio01-exec.sh
...
test_libgpiod_gpio01.cのプログラムリスト
