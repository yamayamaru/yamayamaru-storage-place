...
#define ILI9486_TFTWIDTH  150
#define ILI9486_TFTHEIGHT 200
...
unsigned long testFillScreen() {
  ...
  fillScreen(color16(ILI9341_BLACK));

  fillScreen(color16(ILI9341_RED));

  fillScreen(color16(ILI9341_GREEN));

  fillScreen(color16(ILI9341_BLUE));
...
unsigned long testLines(uint16_t color) {
  ...
  y2    = h - 1;
  start = millis();
  for(x2=0; x2<w; x2+=6) drawLine(x1, y1, x2, y2, color16(color));
  x2    = w - 1;
  for(y2=0; y2<h; y2+=6) drawLine(x1, y1, x2, y2, color16(color));
...
unsigned long testFastLines(uint16_t color1, uint16_t color2) {
  ...
  start = millis();
  for(y=0; y<h; y+=5) drawFastHLine(0, y, w, color16(color1));
  for(x=0; x<w; x+=5) drawFastVLine(x, 0, h, color16(color2));
...
unsigned long testRects(uint16_t color) {
  ...
  fillScreen(color16(ILI9341_BLACK));
  n     = min01(width(), height());
  start = millis();
  for(i=2; i<n; i+=6) {
    i2 = i / 2;
    drawRect(cx-i2, cy-i2, i, i, color16(color));
  }
...
unsigned long testFilledRects(uint16_t color1, uint16_t color2) {
  ...
  for(i=n; i>0; i-=6) {
    i2    = i / 2;
    start = millis();
    fillRect(cx-i2, cy-i2, i, i, color16(color1));
    t    += millis() - start;
    // Outlines are not included in timing results
    drawRect(cx-i2, cy-i2, i, i, color16(color2));
...
unsigned long testFilledCircles(uint8_t radius, uint16_t color) {
  ...
  for(x=radius; x<w; x+=r2) {
    for(y=radius; y<h; y+=r2) {
      fillCircle(x, y, radius, color16(color));
    }
  }
...
unsigned long testCircles(uint8_t radius, uint16_t color) {
  ...
  for(x=0; x<w; x+=r2) {
    for(y=0; y<h; y+=r2) {
      drawCircle(x, y, radius, color16(color));
    }
  }
...
unsigned long testFilledTriangles() {
  ...
  fillScreen(color16(ILI9341_BLACK));
  start = millis();
  for(i=min01(cx,cy); i>10; i-=5) {
    start = millis();
    fillTriangle(cx, cy - i, cx - i, cy + i, cx + i, cy + i,
      color16(color565(0, i*10, i*10)));
    t += millis() - start;
    drawTriangle(cx, cy - i, cx - i, cy + i, cx + i, cy + i,
      color16(color565(i*10, i*10, 0)));
...
unsigned long testRoundRects() {
  ...
  fillScreen(color16(ILI9341_BLACK));
  w     = min01(width(), height());
  start = millis();
  for(i=0; i<w; i+=6) {
    i2 = i / 2;
    drawRoundRect(cx-i2, cy-i2, i, i, i/8, color16(color565(i, 0, 0)));
  }
...
unsigned long testFilledRoundRects() {
  ...
  fillScreen(color16(ILI9341_BLACK));
  start = millis();
  for(i=min01(width(), height()); i>20; i-=6) {
    i2 = i / 2;
    fillRoundRect(cx-i2, cy-i2, i, i, i/8, color16(color565(0, i, 0)));
