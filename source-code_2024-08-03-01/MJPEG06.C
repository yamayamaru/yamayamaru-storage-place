...
// 動画のみで音声は再生されません
//
// 今回のはHigh Cに移植したFM TOWNSのTOWNS OS用32768色版です
// High CでコンパイルできるようにするためにC++からC言語に移植し
// High Cで正常に動作するように修正しました
//
...
//      if(yofset > 0) fillRect(0, yofset + JPEGDecoder_height, _width, _height - (yofset + JPEGDecoder_height), color01(ILI9486_BLACK));
      ...
      if(JPEGDecoder_mjpegflag){
        ...
        fillRect(0, 0, 70, 25, color15(ILI9486_BLACK));
        sprintf(buf01, "fps=%.3f", fps0001);
        draw_string(0, 20, buf01);
