...
// 366行目あたりのloop関数の中身を編集してご自分の動画ファイルを再生するように変更してください
// (拡張子は大文字の.MJPにしてください)
//
...
void setup() {
  ...
  // グラフィック表示の倍率を設定(横2倍、縦2倍にすることで320x240を実現)
  set_screen_ratio(2, 2);

  set_bgcolor(0);
  clear_screen();

  // パレットをWebセーフカラー216色に設定
  for (i = 0; i < 256; i++) {
