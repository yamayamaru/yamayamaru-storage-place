...
sdcc-4.2.0を使ってTeraTermのTEKエミュレーションでメキシカンハットを描いてみました
TeraTermのTEKエミュレーションを使うとマイコンでも手軽にTeraTerm上でグラフィックを描くことができます



hat02_super_aki80_sio1_tek_emu.ihx        コンパイル済みのインテルHEXファイル

hat02_super_aki80_sio1_tek_emu.c          メインプログラムのソース
...
簡単にビルドできるようにhat02_super_aki80_sio1_tek_emu-build.batを用意しました
ファイルの中で環境変数SDCC_HOMEにSDCCをインストールしたフォルダのパスを設定してください
あとはコマンドプロンプトでhat02_super_aki80_sio1_tek_emu-build.batを実行するだけです


TeraTermでTEKエミュレーションを使うにはメニューの設定→端末の画面で
自動切換え(VT<->TEK)のところにチェックを入れます
メニューのコントロール→TEKウインドウを開くを選択してTEKウインドウを開きます
TEKウインドウを適当なサイズにします
TEKウインドウのメニューの設定→ウインドウを開きます
カラーにしたいので16色モード(PC形式)にチェックを入れます
カラーの部分の背景の反転ボタンを押します
