...
extern _Far unsigned char *vram01;
...
_Far unsigned char *vram01;
_Far unsigned char *cvram01;

void gstart(void) {     /* グラフィック表示ON */
    union REGS regs;
    regs.h.ah = 0x40;
    _int86(0x18, &regs, &regs);
...
void set_palette(int color_no, int red, int green, int blue) {
    _outp(0xa8, color_no & 0xff);
    _outp(0xaa, green & 0xff);
    _outp(0xac, red & 0xff);
    _outp(0xae, blue & 0xff);
}
