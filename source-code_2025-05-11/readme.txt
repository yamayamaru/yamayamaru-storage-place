...
Lチカと簡単なシリアル出力のプログラムを作ってみました
PIOのポートBに0.5秒間隔で0から255までの数値を順番に出力します
PIOのポートBにLEDを接続すると1になったbitが光ります
PIOのポートにLEDをつなぐときは必ず電流の流れを制限するための制限抵抗をLEDとポートの間に直列につないでください
制限抵抗をつながないとAKI-80やLEDが壊れます



piob_out_super_aki80.ihx               コンパイル済みのインテルHEXファイル

piob_out_super_aki80.c                 メインプログラムのソース
...
簡単にビルドできるようにpiob_out_super_aki80-build.batを用意しました
ファイルの中で環境変数SDCC_HOMEにSDCCをインストールしたフォルダのパスを設定してください
あとはコマンドプロンプトでpiob_out_super_aki80-build.batを実行するだけです
